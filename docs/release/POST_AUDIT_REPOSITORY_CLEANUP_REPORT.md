...
The controlled `.env.testing` run did not mutate the production database, but it returned 2 failures and 13 errors. Therefore the cleanup validation gate is **BLOCKED_BY_TEST_ENVIRONMENT**, Product writes during the overall cleanup turn cannot truthfully be reported as zero, and no release commit, tag, push, or GitHub Release was created. The project remains safe at the recovered database state, but `v1.25.0` is a prepared release candidate rather than a certified GitHub release.

Commit, tag, push, and GitHub Release results are appended after validation; no credential or secret is recorded here.
