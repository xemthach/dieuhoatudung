...
class ImportDaikinProducts extends Command
{
    protected $signature = 'import:daikin-products {--dry-run : Preview without importing}';
    protected $description = 'Import Daikin products from extracted JSON catalogue data';
    ...
    public function handle(): int
    {
        ...
        $this->info(($dryRun ? '[DRY RUN] ' : '') . "Processing " . count($products) . " Daikin products...");

        $created = $updated = $skipped = 0;
        $errors = [];

        $brand = Brand::where('name', 'Daikin')->first();
        if (!$brand) {
            $this->error('Brand Daikin not found. Please create it first.');
            return 1;
        }
        ...
        foreach ($products as $i => $row) {
            try {
                ...
                if ($dryRun) {
                    ...
                    $this->line("  [{$action}] #{$i} {$row['model_code']}");
                    if ($existing) $updated++;
                    else $created++;
                    continue;
                }
