...
class HVACSeoContentEngine
{
    ...
    public function generate(AIManager $aiManager, AiContentJob $job, string $contextId): array
    {
        ...
        for ($attempt = 1; $attempt <= 5; $attempt++) {
            ...
            try {
                return $this->normalizeOutput($json, $input);
                ...
                $prompt = $this->buildRetryPrompt($input, $json, $e->getMessage());
            }
        }
...
- Có ít nhất một H3, bullet list, ví dụ thực tế, số liệu BTU/m2, so sánh giải pháp và gợi ý sản phẩm.
...
PROMPT;
    ...
    private function buildRetryPrompt(array $input, array $previousJson, string $reason): string
    {
        ...
        return $this->buildPrompt($input).<<<PROMPT
...
- Bài phải đi vào thực tế công trình, có ví dụ m2/BTU, so sánh giải pháp và gợi ý sản phẩm cụ thể.
PROMPT;
    }
...
- Công thức chỉ dùng như ước tính sơ bộ và phải nói cần khảo sát thực tế với công trình lớn.
- Bài phải có tối thiểu 1000 từ, có H2/H3 rõ ràng, ví dụ thực tế, số liệu BTU/m2, so sánh giải pháp và gợi ý sản phẩm.
...
PROMPT;
    ...
    private function validateContentQuality(string $content, array $faq): array
    {
        ...
        if (! preg_match('/\b\d{2,3}[.,]?\d{0,3}\s?BTU\b/iu', $plainText) && ! preg_match('/\b\d+\s?m2\b/iu', $plainText)) {
            $issues[] = 'thiếu số liệu BTU hoặc m2';
        }
